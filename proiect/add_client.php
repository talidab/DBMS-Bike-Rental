<?php
	...
	if(isset($_POST['txtNume']) && isset($_POST['txtPrenume']) && isset($_POST['txtEmail']) && isset($_POST['txtTelefon']) && isset($_POST['txtParola']))
	{
			...
			if (empty($nume)) { 
				header("Location: client_nou.php?error=Introduceti nume!");
				exit();
			}
			else
			if (empty($prenume)) { 
				header("Location: client_nou.php?error=Introduceti prenume!");
				exit();
			}
			else
			if (empty($email)) { 
				header("Location: client_nou.php?error=Introduceti email!");
				exit();
			}
			else
			if (empty($telefon)) { 
				header("Location: client_nou.php?error=Introduceti telefon!");
				exit();
			}
			else
			if (empty($parola)) { 
				header("Location: client_nou.php?error=Introduceti parola!");
				exit();
			}
			else
			{
